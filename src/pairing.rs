...
pub fn generate_pairing_inputs(size: usize) -> (Vec<G1Projective>, Vec<G2Projective>) {
...
pub fn compute_billinearity(g1_vector: Vec<G1Projective>, g2_vector: Vec<G2Projective>) {
    assert!(
        g1_vector.len() == g2_vector.len(),
        "Length of g1 vector and g2 vector should be the same"
    );
